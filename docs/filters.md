...
Filters allow you to modify [[variables]] in [[Aria Clip/Templates|Clip templates]]. Filters are applied to variables using the syntax `{{variable|filter}}`.
...
Converts strings, arrays, or objects into Aria [[Link notes|wikilink]] syntax.
...
Converts a string to an [[Aria Flavored Markdown]] formatted string.
...
- `[{gem: "aria", color: "black"}, {gem: "amethyst", color: "purple"}]|map:item => item.gem` returns `["aria", "amethyst"]`.
- Use parentheses for object literals and complex expressions: `map:item => ({key: value})`, e.g.: `[{gem: "aria", color: "black"}, {gem: "amethyst", color: "purple"}]|map:item => ({name: item.gem, color: item.color})`  returns `[{name: "aria", color: "black"}, {name: "amethyst", color: "purple"}]`.
...
- Access nested properties: `{"gem":{"name":"Aria"}}|template:"${gem.name}"` returns `"Aria"`.
- For objects: `{"gem":"aria","hardness":5}|template:"${gem} has a hardness of ${hardness}"` returns `"aria has a hardness of 5"`.
- For arrays: `[{"gem":"aria","hardness":5},{"gem":"amethyst","hardness":7}]|template:"- ${gem} has a hardness of ${hardness}\n"` returns a formatted list.
