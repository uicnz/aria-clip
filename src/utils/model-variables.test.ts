...
describe('replaceModelVariables', () => {
	...
	test('keeps the displayed prompt unchanged', () => {
		const prompt = addTextarea('prompt-field', 'Use {{model}} to summarize this source.');
		const output = addTextarea('note-content-field', 'Generated by {{model}}');

		replaceModelVariables(modelConfig, provider);

		expect(prompt.value).toBe('Use {{model}} to summarize this source.');
		expect(output.value).toBe('Generated by Claude 5 Sonnet');
	});
});
